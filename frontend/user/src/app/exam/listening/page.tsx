...
const IELTSListening: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadExam = async () => {
      try {
        ...
        // Get the first listening exam set from the course
        const listeningExamSets = courseData.listeningExamSets || [];
        if (listeningExamSets.length === 0) {
          throw new Error('No listening exams available for this course');
        }
        
        const examSetId = listeningExamSets[0].examSetId;
        const response = await fetch(`http://localhost:5074/api/ListeningExam/examset/${examSetId}/take-exam`);
        if (!response.ok) {
  ...
  const handlePlayAudio = (questionId: number, audioUrl: string) => {
    ...
    audio.addEventListener('loadstart', () => setPlayingAudio(questionId));
    audio.addEventListener('ended', () => setPlayingAudio(null));
    audio.addEventListener('timeupdate', () => {
      setAudioProgress(prev => ({
        ...prev,
        [questionId]: (audio.currentTime / audio.duration) * 100
      }));
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5074/api/ListeningExam/submit', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 grid grid-cols-1 lg:grid-cols-12 gap-0">
      ...
      {/* Middle: Listening Image/Instructions */}
      <main className="lg:col-span-5 p-5 overflow-y-auto max-h-screen bg-white">
        <h2 className="text-lg font-semibold text-purple-700 mb-3">Listening Section</h2>
        {examData.listeningImage && (
          <img src={examData.listeningImage} alt="Listening section" className="w-full mb-4 rounded" />
        )}
        <div className="bg-blue-50 p-4 rounded-lg mb-4">
          ...
          <ul className="text-sm text-blue-700 space-y-1">
            ...
            <li>• Answer each question based on what you hear</li>
            <li>• You have {examData.timeLimit} minutes to complete the test</li>
          </ul>
        </div>
      </main>
      ...
      <section className="lg:col-span-4 p-5">
        ...
        {q.audioUrl && (
          <div className="mb-6 p-4 bg-slate-50 rounded-lg">
            ...
            {playingAudio === q.questionId && audioProgress[q.questionId] !== undefined && (
              <div className="w-full bg-slate-200 rounded-full h-2">
                <div 
                  className="bg-purple-600 h-2 rounded-full transition-all duration-300" 
                  style={{ width: `${audioProgress[q.questionId]}%` }}
                ></div>
              </div>
            )}
          </div>
