...
const ListeningExam: React.FC = () => {
  ...
  const [examData, setExamData] = useState<ListeningExamData | null>(null);
  ...
  const loadExamQuestions = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5074/api/ListeningExam/examset/${examSetId}`);
      if (!response.ok) {
      ...
      const transformedData: ListeningExamData = {
        examSetId: data.examSetId,
        examSetTitle: data.examSetTitle,
        examSetCode: data.examSetCode,
        listeningImage: data.listeningImage,
      ...
      const initialAnswers: ListeningAnswer[] = transformedData.questions.map((q: ListeningQuestion) => ({
        questionId: q.questionId,
              ...
              <div className="space-y-4">
                {result.answerResults.map((answerResult, index) => (
                  ...
                  >
                    ...
                    <div className="mt-2">
                      <audio controls src={answerResult.audioUrl} className="w-full">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-lg font-bold text-purple-600">{formatTime(timeSpent)}</div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                ...
                >
                  {isPlaying ? '⏸ Tạm dừng' : '▶️ Phát audio'}
                </button>
              </div>
            </div>
          </div>
                ...
                {currentQuestionData.questionType === 'choice' ? (
                  ...
                  <div>
                    <input
                      ...
                      className="w-full p-3 border rounded focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                    />
                  </div>
                )}
              </div>
